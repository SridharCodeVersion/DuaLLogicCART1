...
class TruthTableVisualizer:
    ...
    def create_pdac_recommendation_card(self, recommended_gate: Dict[str, Any]) -> go.Figure:
        """
        Create a recommendation card specifically for PDAC therapy.

        Args:
            recommended_gate: Best gate recommendation with PDAC context

        Returns:
            Plotly figure object
        """
        gate_name = recommended_gate['gate']
        score = recommended_gate['score']
        explanation = recommended_gate.get('explanation', '')
        safety_note = recommended_gate.get('safety_note', '')
